...
class IRAT_SEPARATE(MultiAgentIrat):
    def __init__(
        self,
        ...
        self._idv_state_preprocessor = self._as_dict(self.cfg["state_preprocessor"])
        self._idv_state_preprocessor_kwargs = self._as_dict(self.cfg["state_preprocessor_kwargs"])
        self._idv_shared_state_preprocessor = self._as_dict(self.cfg["shared_state_preprocessor"])
        self._idv_shared_state_preprocessor_kwargs = self._as_dict(self.cfg["shared_state_preprocessor_kwargs"])
        self._idv_value_preprocessor = self._as_dict(self.cfg["value_preprocessor"])
        self._idv_value_preprocessor_kwargs = self._as_dict(self.cfg["value_preprocessor_kwargs"])
        
        # TEAM policy preprocessor
        self._team_state_preprocessor = self._as_dict(self.cfg["state_preprocessor"])
        self._team_state_preprocessor_kwargs = self._as_dict(self.cfg["state_preprocessor_kwargs"])
        self._team_shared_state_preprocessor = self._as_dict(self.cfg["shared_state_preprocessor"])
        self._team_shared_state_preprocessor_kwargs = self._as_dict(self.cfg["shared_state_preprocessor_kwargs"])
        self._team_value_preprocessor = self._as_dict(self.cfg["value_preprocessor"])
        self._team_value_preprocessor_kwargs = self._as_dict(self.cfg["value_preprocessor_kwargs"])
        ...
        for uid in self.possible_agents:
            ...
            if self._idv_state_preprocessor[uid] is not None:
                self._idv_state_preprocessor[uid] = self._idv_state_preprocessor[uid](**self._idv_state_preprocessor_kwargs[uid])
                self.checkpoint_modules[uid]["idv_state_preprocessor"] = self._idv_state_preprocessor[uid]
            else:
                self._idv_state_preprocessor[uid] = self._empty_preprocessor

            if self._idv_shared_state_preprocessor[uid] is not None:
                self._idv_shared_state_preprocessor[uid] = self._idv_shared_state_preprocessor[uid](
                    **self._idv_shared_state_preprocessor_kwargs[uid]
                )
                self.checkpoint_modules[uid]["idv_shared_state_preprocessor"] = self._idv_shared_state_preprocessor[uid]
            else:
                self._idv_shared_state_preprocessor[uid] = self._empty_preprocessor

            if self._idv_value_preprocessor[uid] is not None:
                self._idv_value_preprocessor[uid] = self._idv_value_preprocessor[uid](**self._idv_value_preprocessor_kwargs[uid])
                self.checkpoint_modules[uid]["idv_value_preprocessor"] = self._idv_value_preprocessor[uid]
            else:
                self._idv_value_preprocessor[uid] = self._empty_preprocessor
                
            # set up team preprocessors
            if self._team_state_preprocessor[uid] is not None:
                self._team_state_preprocessor[uid] = self._team_state_preprocessor[uid](**self._team_state_preprocessor_kwargs[uid])
                self.checkpoint_modules[uid]["team_state_preprocessor"] = self._team_state_preprocessor[uid]
            else:
                self._team_state_preprocessor[uid] = self._empty_preprocessor

            if self._team_shared_state_preprocessor[uid] is not None:
                self._team_shared_state_preprocessor[uid] = self._team_shared_state_preprocessor[uid](
                    **self._team_shared_state_preprocessor_kwargs[uid]
                )
                self.checkpoint_modules[uid]["team_shared_state_preprocessor"] = self._team_shared_state_preprocessor[uid]
            else:
                self._team_shared_state_preprocessor[uid] = self._empty_preprocessor

            if self._team_value_preprocessor[uid] is not None:
                self._team_value_preprocessor[uid] = self._team_value_preprocessor[uid](**self._team_value_preprocessor_kwargs[uid])
                self.checkpoint_modules[uid]["team_value_preprocessor"] = self._team_value_preprocessor[uid]
            else:
                self._team_value_preprocessor[uid] = self._empty_preprocessor
    ...
    def act(self, states: Mapping[str, torch.Tensor], timestep: int, timesteps: int) -> torch.Tensor:
        ...
        with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
            idv_data = [
                self.idv_policies[uid].act({"states": self._idv_state_preprocessor[uid](states[uid])}, role="policy")
                for uid in self.possible_agents
            ]
            ...
            team_data = [
                self.team_policies[uid].act({"states": self._team_state_preprocessor[uid](states[uid])}, role="policy")
                for uid in self.possible_agents
            ]
            ...
            if self._act_policy == "idv":
                ...
                team_log_prob = {}
                for uid in self.possible_agents:
                    state = self._team_state_preprocessor[uid](states[uid])
                ...
                idv_log_prob = {}
                for uid in self.possible_agents:
                    state = self._idv_state_preprocessor[uid](states[uid])
            ...
            for uid in self.possible_agents:
                ...
                with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                    idv_values, _, _ = self.idv_values[uid].act(
                        {"states": self._idv_shared_state_preprocessor[uid](shared_states)}, role="value"
                    )
                    idv_values = self._idv_value_preprocessor[uid](idv_values, inverse=True)
                    
                    team_values, _, _ = self.team_values[uid].act(
                        {"states": self._team_shared_state_preprocessor[uid](shared_states)}, role="value"
                    )
                    team_values = self._team_value_preprocessor[uid](team_values, inverse=True)
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        for uid in self.possible_agents:
            ...
            # compute INDIVIDUAL returns and advantages
            with torch.no_grad(), torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                idv_value.train(False)
                idv_last_values, _, _ = idv_value.act(
                    {"states": self._idv_shared_state_preprocessor[uid](self._current_shared_next_states.float())},
                    role="value",
                )
                idv_value.train(True)
            idv_last_values = self._idv_value_preprocessor[uid](idv_last_values, inverse=True)
            ...
            idv_returns, idv_advantages = compute_gae(
                ...
                last_values=idv_last_values,
                discount_factor=self._discount_factor[uid],
                lambda_coefficient=self._lambda[uid],
            )

            memory.set_tensor_by_name("idv_values", self._idv_value_preprocessor[uid](idv_values, train=True))
            memory.set_tensor_by_name("idv_returns", self._idv_value_preprocessor[uid](idv_returns, train=True))
            memory.set_tensor_by_name("idv_advantages", idv_advantages)
            
            # compute TEAM returns and advantages
            with torch.no_grad(), torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                team_value.train(False)
                team_last_values, _, _ = team_value.act(
                    {"states": self._team_shared_state_preprocessor[uid](self._current_shared_next_states.float())},
                    role="value",
                )
                team_value.train(True)
            team_last_values = self._team_value_preprocessor[uid](team_last_values, inverse=True)
            ...
            team_returns, team_advantages = compute_gae(
                ...
                last_values=team_last_values,
                discount_factor=self._discount_factor[uid],
                lambda_coefficient=self._lambda[uid],
            )

            memory.set_tensor_by_name("team_values", self._team_value_preprocessor[uid](team_values, train=True))
            memory.set_tensor_by_name("team_returns", self._team_value_preprocessor[uid](team_returns, train=True))
            ...
            for epoch in range(self._learning_epochs[uid]):
                ...
                for (
                    ...
                    with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                        ...
                        team_sampled_states = self._team_state_preprocessor[uid](sampled_states, train=not epoch)
                        team_sampled_shared_states = self._team_shared_state_preprocessor[uid](
                            sampled_shared_states, train=not epoch
                        )
                        ...
                        # compute approximate KL divergence
                        with torch.no_grad():
                            team_ratio = team_next_log_prob - team_sampled_log_prob
                            team_kl_divergence = ((torch.exp(team_ratio) - 1) - team_ratio).mean()
                            team_kl_divergences.append(team_kl_divergence)

                        # early stopping with KL divergence
                        if self._kl_threshold[uid] and team_kl_divergence > self._kl_threshold[uid]:
                            break

                        # compute entropy loss
                        if self._entropy_loss_scale[uid]:
                            team_entropy_loss = -self._entropy_loss_scale[uid] * team_policy.get_entropy(role="policy").mean()
                        else:
                            team_entropy_loss = 0
                ...
                if self._learning_rate_scheduler[uid]:
                    if isinstance(self.idv_schedulers[uid], KLAdaptiveLR):
                        idv_kl = torch.tensor(idv_kl_divergences, device=self.device).mean()
                        # reduce (collect from all workers/processes) KL in distributed runs
                        if config.torch.is_distributed:
                            torch.distributed.all_reduce(idv_kl, op=torch.distributed.ReduceOp.SUM)
                            idv_kl /= config.torch.world_size
                        self.idv_schedulers[uid].step(idv_kl.item())
                    else:
                ...
                if self._learning_rate_scheduler[uid]:
                    if isinstance(self.team_schedulers[uid], KLAdaptiveLR):
                        team_kl = torch.tensor(team_kl_divergences, device=self.device).mean()
                        # reduce (collect from all workers/processes) KL in distributed runs
                        if config.torch.is_distributed:
                            torch.distributed.all_reduce(team_kl, op=torch.distributed.ReduceOp.SUM)
                            team_kl /= config.torch.world_size
                        self.team_schedulers[uid].step(team_kl.item())
                    else:
            ...
            if self._learning_rate_scheduler[uid]:
                self.track_data(f"Learning / INDIVIDUAL Learning rate ({uid})", self.idv_schedulers[uid].get_last_lr()[0])
            ...
            if self._learning_rate_scheduler[uid]:
                self.track_data(f"Learning / TEAM Learning rate ({uid})", self.team_schedulers[uid].get_last_lr()[0])