...
class MultiAgentIrat:
    def __init__(
        ...
        for _models in self.idv_models.values():
            for model in _models.values():
                if model is not None:
                    model.to(model.device)
        for _models in self.team_models.values():
            for model in _models.values():
                if model is not None:
                    model.to(model.device)
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if self.cfg.get("experiment", {}).get("wandb", False):
            ...
            try:
                models_cfg = {
                    uid: {k: v.net._modules for (k, v) in self.idv_models[uid].items()} for uid in self.possible_agents
                }
            except AttributeError:
                models_cfg = {
                    uid: {k: v._modules for (k, v) in self.idv_models[uid].items()} for uid in self.possible_agents
                }
