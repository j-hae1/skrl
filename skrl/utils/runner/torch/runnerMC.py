...
class RunnerMC:  # Runner for Multi-Critic agents
    def __init__(self, env: Union[Wrapper, MultiAgentEnvWrapper], cfg: Mapping[str, Any]) -> None:
        ...
        self._models = self._generate_models_MC(self._env, copy.deepcopy(self._cfg))
        ...
        for agent_id in possible_agents:
            ...
            if separate:
                for role in models_cfg:
                    # copy role cfg for printing purposes
                    role_cfg = copy.deepcopy(models_cfg[role])
        ...
        for agent_id in possible_agents:
            for role, model in models[agent_id].items():
                if isinstance(model, nn.ModuleDict):
                    for gname, m in model.items():
                        if hasattr(m, "init_state_dict"):
                            m.init_state_dict(f"{role}_{gname}")
                else:
